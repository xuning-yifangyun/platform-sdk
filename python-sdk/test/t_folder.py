...
class T_Folder(unittest.TestCase):
    ...
    def test_05_restart_folder_from_trash(self):
        res = self.client.folder().recovery_from_trash(folder_ids=[self.test_folder_id])
        self.assertTrue(res['success'])

    def test_06_create_folder(self):
        res = self.client.folder().create("test_create", self.test_folder_id)
        T_Folder.test_create_folder_id = res['id']
        self.assertTrue(res['success'])

    def test_07_re_create_folder(self):
